...
// CODE DESCRIPTION
//   
//         Created by Raistlin.K @ D.E.M.O.N at  19:26  13/06/2012.
...
using System.IO;
using UnSQL.HASH;
using UnSQL.Properties;

#endregion

namespace UnSQL.FireBird
{
    public class Controller
    {
        ...
        public delegate void UninstallDelegate();
        ...
        public event InstallDelegate InstallComplete;
        public event StartDelegate StartComplete;
        public event StopDelegate StopComplete;
        public event UninstallDelegate UninstallComplete;

        public void Install(string iPath, InstallType iType)
        {
            ...
            byte[] iSave = Resources.FirebirdShare;
            ...
            {
                byte[] iSave32 = Resources.FirebirdX86;
            ...
            {
                byte[] iSave64 = Resources.FirebirdX64;
            ...
            //Then we need to install firebird as a windows service depend on user choice.
            string iTempBat = "";
            switch (iType)
            {
                case InstallType.Classic:
                    iTempBat = "install -auto -classic -z";
                    break;
                case InstallType.SuperClassic:
                    iTempBat = "install -auto -classic -multithreaded -z";
                    break;
                case InstallType.Super:
                    iTempBat = "install -auto -superserver -guardian -z";
                    break;
            }
            //We need administrator access here.Otherwise it will be faild.
            //First run instreg install -z
            var p = new ProcessStartInfo
                        {
                            FileName = iPath + "\\bin\\instreg",
                            Arguments = "install -z",
            ...
            var ps = new ProcessStartInfo
                         {
                             FileName = iPath + "\\bin\\instsvc",
                             Arguments = iTempBat,
                             WindowStyle = ProcessWindowStyle.Hidden
                         };

            Process xs = Process.Start(ps);
            xs.WaitForExit();
        ...
        public void Start(string iPath)
        {
            //Deal with "\" for some baby programmer:)
            if (iPath.EndsWith("\\"))
            {
        ...
        public void Stop(string iPath)
        {
            ...
            x.WaitForExit();
            //All finished
            if (StopComplete != null)
            {
                StopComplete();
            }
        }

        public void Uninstall(string iPath)
        {
            //Deal with "\" for some baby programmer:)
            if (iPath.EndsWith("\\"))
            {
            ...
            Process x1 = Process.Start(p1);
            x1.WaitForExit();

            //At the end, we need clear reg info
